...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('users.update',$user->id)}}">
        ...
        <input id="inputTitle" type="text" name="name" placeholder="Ingrese el nombre"  value="{{$user->name}}" class="form-control">
        @error('name')
        ...
        <div class="form-group">
          ...
          <input id="inputEmail" type="email" name="email" placeholder="Ingrese el correo electrónico"  value="{{$user->email}}" class="form-control">
          @error('email')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <select name="role" class="form-control">
                ...
                @foreach($roles as $role)
                    <option value="{{$role->role}}" {{(($role->role=='admin') ? 'selected' : '')}}>Administrador</option>
                    <option value="{{$role->role}}" {{(($role->role=='user') ? 'selected' : '')}}>Usuario</option>
                @endforeach
            </select>
          @error('role')
          <span class="text-danger">{{$message}}</span>
